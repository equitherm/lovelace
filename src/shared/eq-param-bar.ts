...
export class EqParamBar extends LitElement {
  ...
  @property({ type: Boolean }) hideDragTip = false;
  ...
  private _onPointerUp(e: PointerEvent) {
    ...
    this._trackEl = null;
    this.renderRoot.querySelector<HTMLInputElement>('input.sr-only')?.focus();
  }
  ...
  private _updateFromPointer(e: PointerEvent, final: boolean) {
    ...
    if (!final && clamped === this.value) return;
  ...
  render() {
    ...
    if (this.centered) {
      ...
      return html`
        ...
        >
          ...
          ` : nothing}
          ${this._pressed && !this.hideDragTip ? html`
            <span class="drag-tip" style=${styleMap({ '--tip-left': `${valPct}%` })}>${this.value}${this.ariaUnit ? ` ${this.ariaUnit}` : ''}</span>
          ` : nothing}
          ${hiddenRange}
        </div>
      `;
    ...
    return html`
      ...
      >
        ...
        ` : nothing}
        ${this._pressed && !this.hideDragTip ? html`
          <span class="drag-tip" style=${styleMap({ '--tip-left': `${pct}%` })}>${this.value}${this.ariaUnit ? ` ${this.ariaUnit}` : ''}</span>
        ` : nothing}
        ${hiddenRange}
      </div>
    `;
