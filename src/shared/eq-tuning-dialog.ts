...
export class EqTuningDialog extends EquithermBaseElement {
  ...
  private async _applyAll(): Promise<void> {
    ...
    try {
      ...
      if (hcChanged) await this.hass.callService(computeDomain(this.config.hc_entity), 'set_value', { entity_id: this.config.hc_entity, value: this._proposedHc });
      if (shiftChanged) await this.hass.callService(computeDomain(this.config.shift_entity), 'set_value', { entity_id: this.config.shift_entity, value: this._proposedShift });
